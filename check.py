...
def check_script(lines, sp):
    ...
    body = "".join(lines)
    nb = re.sub(r"[\s　]", "", body)
    # →＝「流れ：A → B → C」の手順の区切り。「。」でも切って文ごとに見る（suzuki-ark の演奏編成「A＋B／…／A＋B。同じ曲でも…」を
    #   札に分けて並べたら、区切り記号を隠し文字で残さないと「チェロ。同じ」がつながらず落ちた。隠し文字はコピーすると混ざる）
    SEP = r"[｜|／/　→。]"
    miss = []
    for s in src:
        ...
        # ★枠は「｜」に置き換えて区切りにする。消すだけだと「当日の実施状況は【確認先】でご確認ください」が
        #   「…状況はでご確認ください」とつながり、文ごとに見る時に落ちた（2026-09-14）
        s = re.sub(r"【[^】]*】", "｜", s)
